...
- 鼠标移到可识别的日历事项上，右侧显示小勾选按钮。点击即可标记完成，只有标题出现删除线并降低饱和度与亮度；时间、图标和事项边框保持原色。
...
- 勾选按钮不显示悬浮文字框；屏幕阅读器仍可通过 `aria-label` 和 `aria-pressed` 读出本地完成状态。它不改变 Canvas 原本的完成状态；若 Canvas 自身已经划线，取消本地标记后原生删除线仍会保留。
